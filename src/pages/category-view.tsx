...
export function CategoryViewPage() {
  ...
  const { categories, reload: reloadGlobal } = useVaultData()
  ...
  /* Resolve the category from the shared context */
  useEffect(() => {
    if (!categoryRef || categories.length === 0) return
  ...
  /* Load links for the resolved category */
